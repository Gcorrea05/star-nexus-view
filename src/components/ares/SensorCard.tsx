...
import { severityToToken } from "@/lib/simulation";
  ...
  return (
    <div className="hud-panel rounded-md p-4 transition-colors hover:border-primary/40">
      ...
      <div className="mt-3 flex items-baseline gap-1">
        <span className={cn("text-mono text-3xl font-semibold tabular-nums", `text-${tok}`)}>{value}</span>
      ...
      {progress !== undefined && (
        <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-secondary">
          <div className={cn("h-full transition-all", `bg-${tok}`)} style={{ width: `${Math.max(0, Math.min(100, progress))}%` }} />
        </div>
      )}
    </div>
  );
